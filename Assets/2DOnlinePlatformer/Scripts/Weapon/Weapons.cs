...
{ // Use this for initialization Weapon
    protected bool fire;                      // The player is currently shooting?
    protected Animator anim;                  // Reference to the Animator component.
    public Sprite picture_weapon;             // Sprite weapon
    public GameObject ammunition;             // Ammunition weapon
    public Vector3 spawn_point;               // Point  spawn ammunition weapons
    public bool weapon_animation;             // Animation of recoil of the weapon after a shot from it  
    public bool front;                        // Weapon in front of the player or behind the player's sprite?
    protected Vector3 Spawn_point             // Point  spawn ammunition weapons
    {
        get
        {
    ...
    public int amount;                      // the number of weapons ammunition
    protected int Amount                    // the number of weapons ammunition
    {
        get
        {
            return amount;
    ...
    {// This is all necessary in order to correctly transmit over the network " anim.SetTrigger("Shoot"); ". Example - script Bazooka .
     // In the Update, sometimes the transfer of this SetTrigger over the network does not have time to transmit
        if (fire)
        {
            fire = false;
            if (weapon_animation)
